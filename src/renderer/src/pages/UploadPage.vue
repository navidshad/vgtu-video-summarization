<template>
  <div
    ...
    <div class="container mx-auto px-6 py-12 max-w-7xl flex flex-col h-full z-10 relative">
      <!-- Header -->
      <PageHeader title="FrameFlow"
        subtitle="Select a video or images to begin the AI analysis and extract key insights." :show-back="true">
        <template #back>
      ...
      <div class="flex-1 overflow-y-auto -mx-6 px-6 pb-8 custom-scrollbar flex flex-col items-center">
        ...
        <!-- Tabs -->
        <div v-if="!fileSelected && !isDownloading"
          class="flex bg-zinc-100/50 dark:bg-zinc-900/50 rounded-xl p-1.5 w-full max-w-sm mx-auto mb-6 border border-zinc-200 dark:border-zinc-800 backdrop-blur-sm shadow-sm transition-all duration-300 animate-in fade-in slide-in-from-bottom-2">
          <button @click="uploadMode = 'local'"
            class="flex-1 py-2.5 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2"
            :class="uploadMode === 'local' ? 'bg-white dark:bg-zinc-800 shadow-sm text-zinc-900 dark:text-white scale-[1.02]' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 scale-100'">
          ...
          <button @click="uploadMode = 'link'"
            class="flex-1 py-2.5 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2"
            :class="uploadMode === 'link' ? 'bg-white dark:bg-zinc-800 shadow-sm text-zinc-900 dark:text-white scale-[1.02]' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 scale-100'">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div class="p-10 flex flex-col items-center justify-center min-h-[400px]">
            ...
            <div v-if="!fileSelected && !isDownloading" class="w-full h-full flex-1">
              ...
              <div v-else-if="uploadMode === 'link'"
                ...
                <div
                  class="w-20 h-20 rounded-full bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-500 mb-2 shadow-inner">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path>
                    <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path>
                ...
                <div class="text-center space-y-2 mb-6 w-full">
                  ...
                  <p class="text-base text-zinc-600 dark:text-zinc-400 font-medium">Supports YouTube, Google Drive, and
                    direct
                    media files.</p>
                </div>
                <div class="w-full max-w-md space-y-5">
                  <div class="relative">
                    <input v-model="videoUrl" type="url" placeholder="https://youtube.com/..."
                      class="w-full px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 text-zinc-900 dark:text-white border border-zinc-300 dark:border-zinc-600 rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all shadow-inner pr-12"
                      @keyup.enter="handleLinkStep" :disabled="!ytDlpAvailable || isAnalyzing" />
                  ...
                  <div v-if="availableResolutions.length > 0"
                    ...
                    <div class="flex flex-wrap gap-2">
                      <button v-for="res in availableResolutions" :key="res" @click="selectedResolution = res"
                        class="px-3 py-1.5 rounded-lg text-sm font-bold border transition-all"
                        :class="selectedResolution === res ? 'bg-primary border-primary text-white shadow-md' : 'bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 hover:border-primary/50'">
                        {{ res }}p
                      </button>
                    </div>
                  </div>
            ...
            <div v-else class="w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div
                ...
                <div v-if="isDownloading" class="absolute inset-0 bg-blue-500/5 transition-all duration-300"
                ...
                <div class="flex items-center space-x-5 z-10 flex-1">
                  <div
                    ...
                    <svg v-if="!isDownloading" xmlns="http://www.w3.org/2000/svg" class="w-8 h-8" viewBox="0 0 24 24"
                      fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                      <path d="m22 8-6 4 6 4V8Z" />
                      <rect width="14" height="12" x="2" y="6" rx="2" ry="2" />
                    </svg>
                    <!-- Loading Spinner for download -->
                    <svg v-else class="animate-spin h-8 w-8 text-primary" xmlns="http://www.w3.org/2000/svg" fill="none"
                      viewBox="0 0 24 24">
              ...
              <div class="relative group/btn pt-2">
                ...
                <div v-if="!ffmpegAvailable || (isDownloading && prompt.trim())"
                  ...
                  {{ !ffmpegAvailable
                    ...
                    : 'Summary will be ready once download finishes'
                  }}
                  <div
