<template>
	<div class="h-full group cursor-pointer" @click="$emit('open', thread.id)">
		<Card
			...
			<div class="absolute -right-10 -top-10 w-32 h-32 bg-primary/5 rounded-full blur-3xl group-hover:bg-primary/10 transition-colors duration-500"></div>

			<div class="p-6 flex flex-col h-full relative z-10">
				<div class="flex items-start justify-between mb-6">
					<div
						class="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary group-hover:scale-110 group-hover:rotate-3 transition-all duration-500">
					...
					<div class="flex items-center space-x-2">
						<button @click.stop="$emit('delete', thread.id)"
							class="p-2 rounded-xl text-zinc-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-all opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 duration-300"
							title="Delete Thread">
						...
						<div class="text-[10px] font-bold uppercase tracking-widest text-zinc-400/80 bg-zinc-50 dark:bg-zinc-800/50 px-2 py-1 rounded-lg">
				...
				<div
					...
					<div v-if="thread.versionCounter"
						...
						<span class="font-bold uppercase tracking-wider text-[9px]">v.{{
					...
					<div
						class="flex items-center font-bold text-zinc-900 dark:text-zinc-100 bg-zinc-100/50 dark:bg-zinc-800/50 px-2.5 py-1.5 rounded-lg font-mono text-[9px]">
